...
Make sure you have the Steam version of Helldivers II installed

Both the setup and the launch script assumes that you have it installed in the default Steam path, if you installed it somewhere else modify the `HELLDIVERS_II_PATH` variable in the scripts
...
Download `helldivers-ii-arm64.tar.gz` from the [releases](https://github.com/tranarchy/arm64-linux-games/releases) section

Extract it with `tar -xvf helldivers-ii-arm64.tar.gz`

Run `./setup.sh`

Now you can launch Helldivers II with `helldivers-ii-arm64`
...
cd arm64-linux-games/helldivers-ii
mkdir lib64
